...
export default {
    data() {
        return {
            name: null,
            user_type: 0,
            isLoggedIn: localStorage.getItem('fsShop.jwt') != null
        }
    },
    mounted() {
        this.setDefaults()
    ...
    methods : {
        setDefaults() {
            if (this.isLoggedIn) {
                let user = JSON.parse(localStorage.getItem('fsShop.user'))
                this.name = user.name
        ...
        logout(){
            localStorage.removeItem('fsShop.jwt')
            localStorage.removeItem('fsShop.user')
            this.change()
            this.$router.push('/')
        }
    }
