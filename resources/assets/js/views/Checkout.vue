<template>
 ...
 export default {
     ...
     mounted() {
         this.isLoggedIn = localStorage.getItem('fsShop.jwt') != null
     },
     beforeMount() {
         axios.get(`/api/products/${this.pid}`).then(response => this.product = response.data)

         if (localStorage.getItem('fsShop.jwt') != null) {
             this.user = JSON.parse(localStorage.getItem('fsShop.user'))
             axios.defaults.headers.common['Content-Type'] = 'application/json'
             axios.defaults.headers.common['Authorization'] = 'Bearer ' + localStorage.getItem('fsShop.jwt')
         }
     },
     methods : {
         login() {
